...
package xyz.mp3martin.biggerhotbar.biggerhotbar.updatechecker;
...
class UpdateCheckerMessages {
    ...
    protected static void printCheckResultToPlayer(Player player, boolean showMessageWhenLatestVersion) {
        ...
        if (instance.getLastCheckResult() == UpdateCheckResult.NEW_VERSION_AVAILABLE) {
            ...
            sendLinks(player);
            player.sendMessage(ChatColor.GRAY + "Latest version: " + ChatColor.GREEN + instance.getLatestVersion() + ChatColor.GRAY + " | Your version: " + ChatColor.RED + instance.getUsedVersion());
            player.sendMessage(ChatColor.LIGHT_PURPLE + "    -------");
            player.sendMessage("");
